...
const CARS = [
  ...
  // ... (Other cars remain hidden or for pre-order)
];
        ...
        <div className="flex-1 text-center md:text-left">
          ...
          <div className="mt-8 flex justify-center md:justify-start gap-4">
            <button 
                ...
                style={{backgroundColor: BLUE, color: 'white', hover: {backgroundColor: '#005F9A'}}}
                >
              View Inventory
            </button>
            <button 
                ...
                style={{borderColor: BLUE, color: BLUE, hover: {backgroundColor: 'rgba(0, 119, 182, 0.1)'}}}
                >
              Reserve a Model
            </button>
          </div>
      ...
      <div className="p-5">
        ...
        <div className="mt-4 flex items-center justify-between pt-3 border-t border-gray-100">
          ...
          <div className="flex gap-2">
            <button className={`px-4 py-1.5 text-sm border rounded-full font-semibold transition-colors`} style={{borderColor: BLUE, color: BLUE, hover: {backgroundColor: 'rgba(0, 119, 182, 0.1)'}}}>
              Details
            </button>
            ...
            {car.availableInUSA ? (
                <button 
                  ...
                  style={{backgroundColor: BLUE, hover: {backgroundColor: '#005F9A'}}}
                  >
                  Purchase
                </button>
            ) : (
                <button 
                  className={`px-4 py-1.5 text-sm rounded-full text-white font-semibold transition-colors`}
                  style={{backgroundColor: GRAY_ACCENT, hover: {backgroundColor: '#373A53'}}}
                  >
                  Contact
                </button>
            )}
...
// (PreOrder, About, and Contact components would be included here with the new styling,
// but are omitted for brevity to focus on the Purchase Flow and main App logic.)
...
function PurchaseFlow({ car, flowStep, setFlowStep, purchaseFormData, setFormData, onComplete }) {
  ...
  // Step 1: Handle application form submission -> proceeds to Fee Step (Step 2)
  const handleSubmitStep1 = (e) => {
    e.preventDefault();
    setFlowStep(2); 
  };
  
  // Step 3: Handle crypto selection -> proceeds to Wallet Display Step (Step 4)
  ...
  // Custom Component for the Loading Steps (5 & 9)
  ...
  const renderStepContent = () => {
    switch (flowStep) {
        ...
        return (
          <form onSubmit={handleSubmitStep1} className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="col-span-full flex justify-end">
              <button type="submit" className={`px-6 py-2 text-white rounded-full font-semibold transition`} style={{backgroundColor: BLUE, hover: {backgroundColor: '#005F9A'}}}>
      ...
      case 2: // Step 2: Payment Page Display (New Requirement)
        return (
          <div className="text-center p-6">
            ...
            <div className="inline-block p-4 border-b-4 border-gray-200 mb-6">
                <p className="text-xl text-gray-700">A refundable application fee is required to reserve the vehicle and process paperwork.</p>
                <p className="text-5xl font-extrabold mt-3" style={{color: NAVY}}>${depositAmount.toFixed(2)}</p>
            </div>
            
            <button onClick={() => setFlowStep(3)} className={`mt-6 px-8 py-3 text-white rounded-full font-semibold text-lg shadow-lg transition`} style={{backgroundColor: BLUE, hover: {backgroundColor: '#005F9A'}}}>
                Select Payment Method &rarr;
            </button>
          </div>
        );
        
      case 3: // Step 3: Crypto Selection Dropdown (New Requirement)
        return (
          <div className="text-center p-6">
            ...
            <div className="mt-8">
                <button onClick={() => setFlowStep(2)} className="text-gray-500 hover:text-gray-700 transition">
                    &larr; Back to Fee Summary
                </button>
            </div>
          </div>
        );
        
      case 4: // Step 4: Wallet Display (New Requirement)
        const walletInfo = CRYPTO_WALLETS[selectedCrypto];
        return (
          <div className="text-center p-6">
            <h4 className="font-bold text-xl mb-4" style={{color: BLUE}}>4. Send Payment to Address</h4>
            ...
            <button onClick={() => setFlowStep(5)} className={`mt-8 px-6 py-2 text-white rounded-full font-semibold transition`} style={{backgroundColor: BLUE, hover: {backgroundColor: '#005F9A'}}}>
              I Have Sent the Payment &rarr;
            </button>
          </div>
        );

      case 5: // Step 5: Loading Screen 1
        return <LoadingStep nextStep={6} />;

      case 6: // Step 6: Delivery Info Confirmation
        ...
        return (
          <div className="text-center p-6">
            ...
            <button onClick={() => setFlowStep(7)} className={`mt-6 px-6 py-2 text-white rounded-full font-semibold transition`} style={{backgroundColor: BLUE, hover: {backgroundColor: '#005F9A'}}}>
              Confirm & Proceed to Final Car Payment &rarr;
            </button>
          </div>
        );

      case 7: // Step 7: Final Payment Selection (Re-uses Step 3 and 4 logic)
        return (
          <div className="text-center p-6">
      ...
      case 8: // Step 8: Wallet Display for Final Payment
        const finalWalletInfo = CRYPTO_WALLETS[selectedCrypto];
        return (
          <div className="text-center p-6">
            ...
            <div className="mt-4 p-5 bg-gray-100 rounded-xl border border-blue-300 mx-auto max-w-xl shadow-inner">
              <p className="font-mono text-base break-all text-gray-800">{finalWalletInfo.address}</p>
              <div className="flex justify-center mt-3">
                  <CopyToClipboard text={finalWalletInfo.address} />
              </div>
            </div>
            
            <p className="text-sm text-gray-500 mt-2">Network: {finalWalletInfo.network}</p>

            <button onClick={() => setFlowStep(9)} className={`mt-8 px-6 py-2 text-white rounded-full font-semibold transition`} style={{backgroundColor: BLUE, hover: {backgroundColor: '#005F9A'}}}>
              I Have Sent the Final Payment &rarr;
            </button>
          </div>
        );
        
      case 9: // Step 9: Loading Screen 2
        return <LoadingStep nextStep={10} />;

      case 10: // Step 10: Final Confirmation
        ...
        return (
          <div className="text-center p-6">
            ...
            <button onClick={onComplete} className={`mt-6 px-6 py-2 text-white rounded-full font-semibold transition`} style={{backgroundColor: NAVY, hover: {backgroundColor: GRAY_ACCENT}}}>
              Return to Inventory
            </button>
          </div>
        );
...
export default function App(){ 
  ...
  const [purchaseStep, setPurchaseStep] = useState(0); // 0 = inactive, 1-10 = active
  const [purchaseFormData, setPurchaseFormData] = useState({});

  const startPurchase = (car) => {
    setSelectedCar(car);
    setPurchaseStep(1); // Start the 10-step flow
